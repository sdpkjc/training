...
signed main() {
  ...
  for (int i = 1; i <= nN; i++) {
    addedge(S, i + nN, arr[i], 0);
    addedge(i, T, arr[i], 0);
    addedge(S, i, INF, p);
    if (i + 1 <= nN) addedge(i + nN, i + nN + 1, INF, 0);
    if (i + m <= nN) addedge(i + nN, i + m, INF, f);
    if (i + n <= nN) addedge(i + nN, i + n, INF, s);
  }
  int costa;
  int ans = minCostMaxflow(S, T, costa);
  printf("%lld\n", costa);
}